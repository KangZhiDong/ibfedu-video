...
public class WebUtils {
	...
	public static void deleteCookieDomain(HttpServletRequest request, HttpServletResponse response, String name,
										  String domain) {
		Cookie[] cookies = request.getCookies();
		if ((cookies != null) && (cookies.length > 0))
			for (int i = 0; i < cookies.length; i++)
	...
	public static String getServletRequestUrlParms(HttpServletRequest request) {
		...
		while (parNames.hasMoreElements()) {
			...
			try {
				sbUrlParms.append(parName).append("=").append(URLEncoder.encode(request.getParameter(parName), "UTF-8"))
						.append("&");
