...
import com.ibf.live.dao.website.WebsiteProfileDao;
...
public class WebsiteProfileServiceImpl implements WebsiteProfileService {
	...
	public void updateWebsiteProfile(WebsiteProfile websiteProfile) throws Exception {
		...
		RedisCacheUtil.del(CacheConstans.WEBSITE_PROFILE+websiteProfile.getType());
	}
	...
	public Map<String, Object> getWebsiteProfileList() throws Exception {
		...
		List<String> websitesStr = gson.fromJson(RedisCacheUtil.get(CacheConstans.WEBSITE_PROFILE),new TypeToken<List<String>>() {}.getType());
		if(ObjectUtils.isNull(websitesStr)||websitesStr.size()==0){
			...
			RedisCacheUtil.set(CacheConstans.WEBSITE_PROFILE, gson.toJson(websitesStr), CacheConstans.WEBSITE_PROFILE_TIME);
		}
	...
	public Map<String, Object> getWebsiteProfileByType(String type) {
		...
		String websiteProfileStr= RedisCacheUtil.get(CacheConstans.WEBSITE_PROFILE+type);
		if(ObjectUtils.isNull(websiteProfileStr)){//cache为空查询数据库
			...
			RedisCacheUtil.set(CacheConstans.WEBSITE_PROFILE+type, websiteProfileStr, CacheConstans.WEBSITE_PROFILE_TIME);//设置key 时间一天
		}
		WebsiteProfile websiteProfile=gson.fromJson(websiteProfileStr,WebsiteProfile.class);
